# Implementation Logs

## Purpose

This index points to the implementation logs that record what has actually been built so far.

Use these logs for handoff and verification.
Use the other docs for product intent, locked architecture, and phase definitions.

## Logs

- [Current Implementation Status](./implementation-status.md)
- [Phase 4: System VPN Configuration Layer](./worklogs/2026-04-05-phase-4-system-vpn-config.md)
- [Phase 5: Menu Bar Connection Controls And State](./worklogs/2026-04-05-phase-5-menu-bar-state.md)
- [Phase 6: Packet Tunnel Startup Contract](./worklogs/2026-04-05-phase-6-packet-tunnel-startup.md)
- [Phase 7: OpenVPN Bridge Slice](./worklogs/2026-04-05-phase-7-openvpn-bridge-slice.md)
- [2026-04-07: Embedded Manager Profile Config Refactor](./worklogs/2026-04-07-embedded-manager-profile-config.md)
- [2026-04-07: Connection Status And Tunnel Startup Fix](./worklogs/2026-04-07-connection-status-timeout-fix.md)
- [2026-04-07: Beta Release Prep](./worklogs/2026-04-07-beta-release-prep.md)
- [2026-04-09: App Store Network Extension Permission Fix](./worklogs/2026-04-09-app-store-networkextension-permission-fix.md)
